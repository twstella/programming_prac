...
public class Tmp {
   ...
   static int xP, yP, xQ, yQ;
   static int cnt, olap;
   
   static void Twist(int x1, int y1, int x2, int y2) {
      long t1, t2, t3, t4;
      //-----------------------------------------
      t1 = CCW(x1, y1, x2, y2, xP, yP);
      t2 = CCW(x1, y1, x2, y2, xQ, yQ);
      t3 = CCW(xP, yP, xQ, yQ, x1, y1);
      t4 = CCW(xP, yP, xQ, yQ, x2, y2);
...
//System.out.println(t1+ " " + t2+ " "+t3+ " "+t4);
   ...
   public static int CheckPt() {
      ...
      if (xP>= xQ) { xL= xP; xS = xQ;}
      else { xL= xQ; xS = xP;};
      if (yP>= yQ) { yL= yP; yS = yQ;}
      else { yL= yQ; yS = yP;};
      
      if ( ((xP == xmin) && (xQ == xmin)) || ((xP == xmax) && (xQ == xmax))) {
         if (yL < ymin) return 0;
         if (yS > ymax) return 0;
      ...
      if ( ((yP == ymin) && (yQ == ymin)) || ((yP == ymax) && (yQ == ymax))) {
         if (xL < xmin) return 0;
         if (xS > xmax) return 0;
